...
def plot_full_preds(x, y, z, avg_preds, idxs, confidance_threshold_for_mean, scale_rate=50):
    ...
    ax.plot(y, label='y')
    ax.plot(z, label='z')
    ax.plot((avg_preds * scale_rate), label=f'Mean probability estimation of medication adherence gesture (scale: 0-{scale_rate})')
    ax.hlines([(confidance_threshold_for_mean * scale_rate)], xmin=0, xmax=len(x), linestyles='dashdot', label=f'Detection threshold: {confidance_threshold_for_mean}')

    for idx_pair in idxs:
        start_idx, end_idx = idx_pair
        plt.axvspan(start_idx, end_idx, color='darkgray', alpha=0.9)
        plt.text(start_idx, plt.ylim()[1]*0.9, f"bout", fontsize=4, rotation=90, backgroundcolor='white', alpha=0.9)

    # Convert centiseconds to hours on x-axis
    hours_conversion = 0.01 / 3600  # Convert from centiseconds to hours
    x_ticks_positions = np.linspace(0, len(x), 6)
    x_ticks_labels = [f"{position * hours_conversion:.2f}" for position in x_ticks_positions]
